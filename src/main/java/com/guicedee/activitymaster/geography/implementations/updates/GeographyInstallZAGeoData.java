...
@SortedUpdate(sortOrder = 1700, taskCount = 1)
@Log4j2
public class GeographyInstallZAGeoData implements ISystemUpdate
{
	...
	{
		log.info("Starting towns and cities loading for Geography Master");
		return SessionUtils.<Boolean>withActivityMaster(enterprise.getName(), GeographySystemName, tuple -> {
			...
			return geographyService.loadTownsAndCities(amSession, amSystem, amToken)
				.chain(() -> {
					wipeCaches();
